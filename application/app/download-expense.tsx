...
import { downloadExpensesFile } from '@/services/file.service'; // your expense API
...
export default function DownloadExpenseScreen() {
  ...
  const openDatePicker = (type: 'from' | 'to') => { setPickerType(type); setShowPicker(true); };
  ...
  const clearError = (key: string) => setErrors((prev) => { const { [key]: _, ...rest } = prev; return rest; });
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      await downloadExpensesFile(params);
    } catch (error: any) {
    ...
    <View style={styles.root}>
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
        <View style={[styles.header, { paddingTop: insets.top + 10 }]}>
          ...
          <Text style={styles.headerTitle}>Share Expense Report</Text>
          <View style={{ width: 40 }} />
        </View>
